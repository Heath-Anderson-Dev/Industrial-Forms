			...
			if (isset($_SESSION['logged']) && $_SESSION['logged'] == true ) {
				...
				else
				{
					...
					if ($image['error'] == NULL) {
						...
						sprintf("INSERT INTO `gallery` (`id`, `title`, `slug`, `parent`, `image`) VALUES ('','%s','%s','%s','%s')",
						mysqli_real_escape_string($connection,$title),
						mysqli_real_escape_string($connection,$slug),
						mysqli_real_escape_string($connection,$parent),
						mysqli_real_escape_string($connection,$img))))
						{
							$last_id = $connection->insert_id;
							$res = @$connection->query(sprintf("UPDATE `gallery` SET `ord` = '".$last_id."' WHERE `id` ='".$last_id."';"));
							$self->result = null;
							$self->status = true;
							$self->error = null;
						} else {
